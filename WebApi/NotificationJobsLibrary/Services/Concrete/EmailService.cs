    ...
    public class EmailService : IEmailService
    {
        ...
        private Task<SendNotificationResult> SendMessage(MailMessage mail)
        {
            try
            {
                ...
                client.Send(mail);
                return Task.FromResult(new SendNotificationResult(){IsSuccessful = true});
            }
            catch (Exception exp)
            {
                return Task.FromResult(new SendNotificationResult(){IsSuccessful = false, LogMessage = exp.Message});
            }
        }

        public async Task<SendNotificationResult> SendMessage(string email, string subject, string body)
        {
            var message = MailMessageCreator.GenerateMessage(email, options.Value.From, subject, body, Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), "Templates", "Resources"));
